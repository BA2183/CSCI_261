...
/**
 * @brief use binary search to find the index of the targeted value
 * 
 * @tparam T 
 ...
 */
template<typename T>
int LinkedList<T>::find(const T targetValue) {
    int startPos = 0, endPos = mSize - 1, targetPos = -1, middlePos = (endPos - startPos) / 2 + startPos;
    
    while(true) {
        // do search
        if(endPos < startPos) {
            break;
        } else if(this->at(middlePos) == targetValue) {
            targetPos = middlePos;
            break;
        } else if(this->at(middlePos) > targetValue) {
            endPos = middlePos - 1;
